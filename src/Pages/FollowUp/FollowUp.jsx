...
const FollowUp = () => {
  ...
  useEffect(() => {
    if (selectedCampaign && emp) {
      dispatch(getFollowUpDetailsThunk(requestData));
  ...
  const fetchLeads = () => {
    ...
    if (hasFilters) {
      if ((["admin", "Admin", "ADMIN"].includes(storedUsername))) {
        dispatch(getFollowUpDetailsThunk({ ...filters, employeeCode: employeeCode === "All" ? "" : employeeCode, pageNumber: currentPage, limit: itemsPerPage }));
      }
      else {
        dispatch(getFollowUpDetailsThunk({ ...filters, employeeCode: emp === "All" ? "" : emp, pageNumber: currentPage, limit: itemsPerPage }));
      }
    }
    // if (hasFilters) {
                  ...
                  {loading ? (
                    ...
                    followUpData.map((followUpObj) => (
                      <tr key={followUpObj.lead?.leadId}>
                        ...
                        <td>{followUpObj.lead.followupDetail?.followUpDateTime || "N/A"}</td>
