...
import { createPoolAccessThunk, DeletePoolAccessThunk, getAllPoolAccessThunk } from "../../../Redux/Services/thunks/PoolAccessThunk";
...
const PoolAccess = () => {
  ...
  const [newPoolRatio, setNewPoolRatio] = useState(0);
  ...
  const { data, loading, error } = useSelector((state) => state.poolAccess);

  //!----------------------------------------------------------------------------------------------<---Pagination Logic------------->------------------------------------------------------
  const totalPages = Math.ceil(Pool.length / itemsPerPage);
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentStatuses = Pool.slice(indexOfFirstItem, indexOfLastItem);
  ...
  useEffect(() => {
    dispatch(getAllPoolAccessThunk());
  }, [dispatch]);
  ...
  const segmentOptions = [
    { value: "Fresh Pool", label: "Fresh Pool" },
    { value: "Diamond Pool", label: "Dimond Pool" },
    { value: "Platinum Pool", label: "Platinum Pool" },
    { value: "HNI Pool", label: "HNI Pool " },
    { value: "WebLeads Pool", label: "WebLeads Pool" },
    { value: "Dispose Pool", label: "Dispose Pool" },
  ];
  ...
  const timePeriodOptions = [
    ...
    { value: "TL", label: "TL" },
    { value: "PTL", label: "PTL" },
    { value: "SBDE", label: "SBDE" },
    { value: "BDE", label: "BDE" },
  ];

  // Handle adding a new plan
  const handleAddPlan = (e) => {
    if ( newPoolName && newDesignation && newPoolRatio) {
      const newPlan = {
        poolName: newPoolName,
        designation: newDesignation,
        poolRatio: newPoolRatio,
      
      };

      dispatch(createPoolAccessThunk(newPlan)).then((response) => {
        setMsg(response?.payload?.message || "Added successfully");
        setPool([...Pool, newPlan]);
        setNewPoolName("");
        setNewDesignation("");
        setNewPoolRatio(0);
        setShowPopup(false);
  ...
  const handleDeleteSegmentPlan = (id) => {
    dispatch(DeletePoolAccessThunk(id))
      .unwrap()
      .then((response) => {
        setMsg(response.message || "deleted successfully");
        setPool((prevStatuses) =>
...
{/* //!--------------------------------------------------------------------------------POPUP INPUT LOGIC HERE-------------------------------------------------------------------------- */}
                        ...
                        <div className="card-body">
                          ...
                          >
                            ...
                            <input
                              type="text"
                              value={newPoolRatio}
                              onChange={(e) => setNewPoolRatio(e.target.value)}
  ...
  {loading ? (
    ...
    Pool?.map((planObj) => (
      <tr key={planObj?.id}>
        <td>{planObj?.id ?? "N/A"}</td>
        <td>{planObj?.poolName ?? "N/A"}</td>
        <td>{planObj?.designation ?? "N/A"}</td>
        <td>{planObj?.poolRatio ?? "N/A"}</td>
        <td>
          <div className="d-flex justify-content-center align-items-center gap-2">
            <EditButton
              ...
              onClick={() => {
                ...
                setNewSegment(planObj?.poolName);
                setNewAmount(planObj?.poolRatio);
                setNewTimePeriod(planObj?.designation);
                setEditValue(planObj?.poolName);
                setEditAmount(planObj?.poolRatio);
                setEditTimePeriod(planObj?.designation);
              }}
            />
            <DeleteButton
